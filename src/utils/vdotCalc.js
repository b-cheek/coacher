export var Formula = {
  ...
  getVDOT: function (distance, time) {
    ...
    var F =
      0.8 +
      0.298956 * Math.exp(-0.193261 * time) +
      0.189439 * Math.exp(-0.012778 * time);
    return VO2 / F;
  },
  ...
  getPredictedRaceTime: function (VDOT, distance) {
    ...
    for (var i = 0; i < 3; i++) {
      var B = Math.exp(-0.193261 * A);
      var C = 0.298956 * B + Math.exp(-0.012778 * A) * 0.189439 + 0.8;
      var E = Math.pow(VDOT * C, 2) * -0.0075 + VDOT * C * 5.000663 + 29.54;
      var G = 0.298956 * B * 0.19326;
      var H = G - Math.exp(-0.012778 * A) * 0.189439 * -0.012778;
      var I = C * H * VDOT * -0.007546 * 3;
  ...
  getEasyPace: function (vdot, distance, slowerPace) {
    ...
    var percentage = slowerPace ? 0.62 : 0.7;
  ...
  getThresholdPace: function (vdot, distance) {
    ...
    return this._getCustomEffortPace(vdot, distance, 0.88);
  },
  ...
  getIntervalPace: function (vdot, distance) {
    ...
    return this._getCustomEffortPace(vdot, distance, 0.975);
  },
  ...
  getCustomTrainingPace: function (
    paceType,
    percentage,
    distanceMeters,
    vdot,
  ...
  _getVO2: function (V) {
    return 0.182258 * V + 0.000104 * Math.pow(V, 2) - 4.6;
  },

  _getPaceVelocity: function (O) {
    return 29.54 + 5.000663 * O - 0.007546 * Math.pow(O, 2);
  },
...
export var FormulaHelpers = {
  ...
  getQualitySetItemPaces: function (
    user,
    distance,
    distanceUnit,
    effort,
    ...
    const paceUnit = isRepetition
      ? userPreferredUnit
      : this.getPaceUnitForUser(distanceUnit, user);
    ...
    const repDistance = Conversion.isTimeUnit(distanceUnit)
      ? paceDistance
      : Conversion.fromDistanceUnitString(distance, distanceUnit, true, false);

    var pace = 0;
    var repTime = 0;

    if (customPace != null) {
      pace = Formula.getCustomTrainingPace(
        customPace.paceType,
        customPace.percentage,
        customPace.distanceMeters,
        user.VDOT,
        paceUnit
      );
      ...
      pace = FormulaHelpers.getQualitySessionPace(
        sessionType,
        user.VDOT,
        paceDistance
      );
  ...
  getSlowPaceForEasyWorkoutSet: function (user, pace, paceUnit) {
    const paceDistance = this.getPaceDistanceForUnit(paceUnit);
    const easyRange = Formula.getEasyPaceRange(
      user.VDOT,
      paceDistance,
      paceUnit
    );
  ...
  getQualityWorkoutRecoveryMeters: function (
    repsCount,
    recoveryAmount,
    recoveryUnit,
    vdot
  ) {
    var recoveryMeters = this.getQualityWorkoutSingleRecoveryMeters(
      repsCount,
      recoveryAmount,
      recoveryUnit,
      vdot
  ...
  getQualityWorkoutSingleRecoveryMeters: function (
    repsCount,
    recoveryAmount,
    recoveryUnit,
    vdot
    ...
    if (Conversion.isTimeUnit(recoveryDistanceUnit)) {
      return Conversion.getRecoveryDistanceFromTime(
        recoveryAmount,
        recoveryUnit,
        vdot
      );
  ...
  isWorkoutSet: function (sessionType) {
    ...
    return (
      value !== QualitySessionType.Rest && value !== QualitySessionType.Text
    );
  },
